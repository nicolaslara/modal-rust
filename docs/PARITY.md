...
| `@asgi_app` / `@wsgi_app` / `@web_server` | ❌ | `_partial_function.py:413` / `468` / `525`. `#[web_server]` (a Rust `Router` behind `WEBHOOK_TYPE_WEB_SERVER`) is the designed follow-up — it reuses `#[endpoint]`'s `webhook_config` plumbing; `#[asgi_app]` (a *Python* ASGI callable) almost certainly never maps to Rust. |
