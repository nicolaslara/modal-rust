...
| `enable_memory_snapshot` (807) | **Have** (`#[cls]`, CPU) | `#[cls(enable_memory_snapshot = true)]` — the expensive `#[enter]` load runs **once ever** on a DEPLOYED app, Modal snapshots the loaded process, and every later (even cold) container RESTORES it instead of re-running the build. DEPLOY-ONLY: the flag rides into `Function.checkpointing_enabled`/`is_checkpointing_function` (proto 41/40) only at the deploy boundary; RUN stays wire-identical. A deploy bakes `MODAL_RUST_SNAPSHOT_PRIME=1` so the wrapper primes `#[enter]` inside the snapshot window. `#[function(enable_memory_snapshot)]` is a `compile_error` (cls-only in v0). `examples/snapshot-class`. Remaining gap: the GPU snap/restore split (a `restore` frame + `#[restore]` hook) — see §8. Modal `snapshot.py`. |
