//! The runner binary for the autoscaling example — the WHOLE thing, one line.
//!
...
//! shows the autoscaler knobs (min/max/buffer containers + scaledown window) riding on
//! the entrypoint's config. The user never writes `main()`.

modal_rust::modal_runner!(example_autoscaling);
