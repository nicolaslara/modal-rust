//! The runner binary for the cpu-memory example — the WHOLE thing, one line.
//!
...
//! shows the `cpu`/`memory` knobs (as `milli_cpu`/`memory_mb`) riding on the
//! entrypoint's config. The user never writes `main()`.

modal_rust::modal_runner!(example_cpu_memory);
