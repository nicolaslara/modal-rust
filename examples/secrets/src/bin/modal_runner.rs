//! The runner binary for the secrets example — the WHOLE thing, one line.
//!
...
//! shows the attached secret riding on the entrypoint's config. The user never
//! writes `main()`.

modal_rust::modal_runner!(example_secrets);
