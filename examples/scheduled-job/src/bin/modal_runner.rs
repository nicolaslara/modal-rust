//! The runner binary for the scheduled-job example — the WHOLE thing, one line.
//!
...
//! shows the `schedule` knob riding on the entrypoint's config. The user never writes
//! `main()`.

modal_rust::modal_runner!(example_scheduled_job);
