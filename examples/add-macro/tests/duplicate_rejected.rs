//! Proves `modal_rust::registry_from_inventory()` rejects duplicate entrypoint names with the
...
//! facade inventory collection panics with the frozen "duplicate entrypoint" message.
...
use modal_rust::{FunctionConfig, HandlerFn, Registration, RunnerError};
...
fn from_inventory_rejects_duplicate_names() {
    let err = match std::panic::catch_unwind(modal_rust::registry_from_inventory) {
