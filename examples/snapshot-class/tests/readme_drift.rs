...
//! the quickstart and stateful-class guards.
//!
//! Mechanism: the README tags the authored `#[cls]` block as ```` ```rust cls ```` and
//! this crate's `src/lib.rs` brackets the same region (`pub struct Concordance` through
//! the end of the `#[cls] impl Concordance` block) with `// cls:begin` / `// cls:end`
//! markers. The test extracts both and asserts byte-equality (after trimming surrounding
//! blank lines). Because the crate compiles + its tests pass, "the README equals the crate
...
fn crate_cls_block() -> String {
    ...
    .expect("read snapshot-class/src/lib.rs");
    let begin = "// cls:begin";
    let end = "// cls:end";
...
fn readme_cls_matches_crate_source() {
    ...
    assert_eq!(
        ...
        "\n\nREADME `#[cls]` block has DRIFTED from examples/snapshot-class/src/lib.rs.\n\
